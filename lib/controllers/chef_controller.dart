...
class GachaEntity {
  ...
  int get tokensNeededToUpgrade {
    ...
    if (isChef) {
      ...
      int multiplier = rarity == GachaRarity.Legendary
          ? 10
  ...
  double get currentFireRate =>
      math.max(0.05, baseFireRate * (1 - (level * 0.02)));
...
class ChefController extends ChangeNotifier {
  ...
  int _activeKnifeIndex =
      -1; // -1 means no specific knife equiped, but let's default to first knife
  ...
  void _initializeData() {
    allEntities = [
      ...
      GachaEntity(
        id: 'k6',
        name: 'Excalibur de Acero',
        rarity: GachaRarity.Legendary,
        icon: Icons.whatshot,
        isChef: false,
        lore: 'Corta incluso el tejido del espacio-tiempo.',
        favoredLocation: '',
        strongAgainst: '',
        weakAgainst: '',
        baseDamage: 50.0,
    ...
    for (var c in ChefDatabase.allChefs) {
      ...
      switch (c.rarity) {
        case ChefRarity.R:
          mappedRarity = GachaRarity.Common;
          break;
        case ChefRarity.SR:
          mappedRarity = GachaRarity.Rare;
          break;
        case ChefRarity.SSR:
          mappedRarity = GachaRarity.Epic;
          break;
        case ChefRarity.UR:
          mappedRarity = GachaRarity.Legendary;
          break;
      }
      ...
      allEntities.add(
        GachaEntity(
          ...
          favoredLocation:
              c.element.name, // Use element as location for UI synergy
          ...
          isUnlocked:
              c.rarity == ChefRarity.R &&
              c.id == 'r_fire_1', // Unlocked default
        ),
      );
    }
  }
  ...
  // GACHA SYSTEM LOGIC
  List<RollResult> rollGacha(
    bool isChefRoll,
    int amount, [
    String chestInfo = "",
    dynamic ecoController,
  ]) {
    final rand = math.Random();
    List<RollResult> results = [];

    // Filter relevant pool
    List<GachaEntity> pool = allEntities
        .where((e) => e.isChef == isChefRoll)
        .toList();
      ...
      if (isChefRoll) {
        ...
        if (wasUnlocked) {
          if (rolled.rarity == GachaRarity.Common)
            ...
            tokensGranted = 15;
          else if (rolled.rarity == GachaRarity.Legendary)
            tokensGranted = 50;
  ...
  double getTotalDamage(String location) {
    ...
    if (activeChef.favoredLocation == location ||
        activeChef.favoredLocation == elemLocation) {
      d *= 1.5;
    }
    return d;
  }

  // Grants a portion of tokens for a random chef when a boss is defeated (15% chance).
  void grantBossTokens() {
    // 15% probability
    ...
    debugPrint('Boss dropped $amount tokens for ${randomChef.name}!');
    notifyListeners();
  }
}
