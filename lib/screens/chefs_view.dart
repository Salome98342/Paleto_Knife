              ...
              Expanded(
                child: DropdownButton<GachaRarity?>(
                  ...
                  items: [
                    ...
                    ...GachaRarity.values.map(
                      (r) => DropdownMenuItem(
                        value: r,
                        child: Text(
                          r.name.toUpperCase(),
                          style: TextStyle(color: _getRarityColor(r)),
                        ),
                      ),
                    ),
                  ],
          ...
          children: [
            ...
            GridView.count(
              ...
              children: entry.value
                  .map((e) => _buildChefCard(context, e, chefState, isChef))
                  .toList(),
            ),
      ...
      onTap: () {
        if (!entity.isUnlocked) {
          RetroStyle.showInsufficient(
            context,
            "${isChef ? 'CHEF' : 'CUCHILLO'} BLOQUEADO.\nCONSIGUELO EN\nLA TIENDA GACHA.",
          );
          return;
        }
        try {
      ...
      child: Stack(
        children: [
          Container(
            decoration:
                RetroStyle.box(
                  color: isActive ? Colors.grey.shade800 : RetroStyle.panel,
                ).copyWith(
                  border: Border.all(
                    color: isActive ? Colors.amber : rarityColor,
                    width: isActive ? 6 : 4,
                  ),
                ),
            child: Column(
              ...
              children: [
                ...
                Text(
                  "Lv. ${entity.level}",
                  style: RetroStyle.font(size: 8, color: rarityColor),
                ),
      ...
      builder: (BuildContext dialogContext) {
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            final activeEntity = isChef
                ? cController.activeChef
                : cController.activeKnife;
            ...
            bool canUpgrade =
                (upgradeCost > 0) &&
                (currentEntity.tokens >= upgradeCost) &&
                (eco.coins >= coinCost) &&
                !isMaxLevel;
            ...
            return Dialog(
              ...
              child: Container(
                ...
                decoration: RetroStyle.box(color: RetroStyle.panel).copyWith(
                  border: Border.all(
                    color: _getRarityColor(currentEntity.rarity),
                    width: 4,
                  ),
                ),
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Icon(
                              ...
                              .animate(
                                key: ValueKey(currentEntity.level),
                              ) // Reactiva animación de scale si sube de nivel
                              .scaleXY(
                                begin: 1.5,
                                end: 1.0,
                                duration: 400.ms,
                                ...
                                Row(
                                  children: [
                                    Text(
                                        ...
                                        .animate(
                                          key: ValueKey(
                                            "level_${currentEntity.level}",
                                          ),
                                        )
                                        .slideY(begin: -0.5, duration: 300.ms),
                                    Text(
                                      "| ${currentEntity.rarityName}",
                                      style: RetroStyle.font(
                                        size: 8,
                      ...
                      _buildStatRow(
                        "Daño Base:",
                        currentEntity.currentDamage.toStringAsFixed(1),
                        currentEntity.level,
                      ),
                      _buildStatRow(
                        "Vel. Ataque:",
                        "${currentEntity.currentFireRate.toStringAsFixed(2)}s",
                        currentEntity.level,
                      ),
                      const SizedBox(height: 16),
                      ...
                      if (isChef) ...[
                        _buildElementPanel(
                          "Mejor Zona",
                          currentEntity.favoredLocation,
                          Colors.blueAccent,
                        ),
                        _buildElementPanel(
                          "Fuerte contra",
                          currentEntity.strongAgainst,
                          Colors.green,
                        ),
                        _buildElementPanel(
                          "Débil contra",
                          currentEntity.weakAgainst,
                          Colors.redAccent,
                        ),
                        const SizedBox(height: 24),
                      ],
                              ...
                              onTap: () {
                                if (canUpgrade) {
                                  ...
                                  if (currentEntity.tokens < upgradeCost) {
                                    RetroStyle.showInsufficient(
                                      context,
                                      "FALTAN ${upgradeCost - currentEntity.tokens} TOKENS",
                                    );
                                  } else {
                                    RetroStyle.showInsufficient(
                                      context,
                                      "FALTAN ${coinCost - eco.coins} MONEDAS",
                                    );
                                  }
                                }
                              },
      ...
      child: Row(
        children: [
          Expanded(
            ...
            child: Text(
              "$title:",
              style: RetroStyle.font(size: 8, color: color),
            ),
          ),
          Expanded(
            flex: 3,
